...
{	
	...
	JoinBitmapIndexProbes(pmp, pdrgpexprBitmap, pdrgpexprRecheckNew, &pexprBitmapBoolOp, &pexprRecheckNew, fConjunction);
...
{	
	...
	// decide whether to break the predicate down and look for index paths
	// on the individual children.
	// we do not break the predicate down in following two scenarios to
	// generate optimal index paths :
	// 1. predicates of the form <ident> op <const>
	// 2. conjunct tree with children of the form <ident> op <const>
	//    or <boolean ident> or NOT <boolean ident>. This pattern is
	//    checked in FAndWithScalarIdentToScalarConstChildren() below.
	//    Not breaking such conjuncts gives us an opportunity to find
	//    indexes that cover maximum columns used in the predicate:
	...
	// for rest of the expression types, such as <ident> op <ident>,
	// disjuncts or a conjunct containing any of these, look for index
	// access paths in its children
	if (!CPredicateUtils::FAndWithScalarIdentToScalarConstChildren(pexprPred))
	{
		(*retryIndexLookupWithResidual) = false;

		// first, check if it is an index lookup predicate
		// if the predicate is of the form <ident> op <ident>
		...
		// if no index path was constructed for this predicate, return
		// it as residual.
		// Example, with schema: t(a, b, c, d), index i1(a,b)
		// and predicate: (a = 3) OR (c = 4 AND d =5)
		// no index path will be found for (c = 4 AND d =5)
		// in which case the entire disjunct will become a residual
	...
	for (ULONG ul = 0; ul < ulIndexes; ul++)
	{
									...
									))
		{
			...
			if (minResidual > ulResidualLength)
			{
				...
				// discard the trivial filter that gets generated when index covers all the columns,
				// in which case the residual is const true
...
{
	GPOS_ASSERT(NULL != pdrgpexpr);
	CExpression *pexprRecheck, *pexprResidual;

	ULONG ulLength = pdrgpexpr->UlLength();
	...
	for (ULONG ul = 0; ul < ulLength; ul++)
	{
		...
		// if an index path was found only for a part of the predicate,
		// then retryIndexLookupWithResidual decides whether to perform
		// the index lookup again for the residual
		BOOL retryIndexLookupWithResidual = false;

		// perform multiple index lookups to minimize the number of residuals.
		...
		{
			pexprRecheck = pexprResidual = NULL;
			...
			if (NULL != pexprBitmap)
			{
				...
				// if the original predicate is a conjuct and if the matched index has already
				// been selected previously, then merge them
				// Example, for the schema: t1(c1), t2(c2) and index i1(t2.c2)
				// and query: select * from t1, t2 where t1.c1 = t2.c2 and t2.c2 = 10;
				// in case of Join2Index apply xform, i1 will be selected for both the predicates,
				// hence can be merged
				...
				{
					pdrgpexprRecheckTemp->Append(pexprRecheck);
					pdrgpexprBitmapTemp->Append(pexprBitmap);
				}
			}

			if (NULL != pexprBitmap && retryIndexLookupWithResidual)
			{
				// if an index path was found, then perform the
				// index lookup again for the residual
				pexprPred = pexprResidual;
				continue;
			}

			// terminate the lookup if no index path was found or
			// if the retry was not allowed
			pexprPred = NULL;
		}
		...
		// for conjuncts, there may be multiple index paths genearted due to the
		// retry, combine them with BitmapAnd
		const ULONG ulBitmapExpr = pdrgpexprBitmapTemp->UlLength();
		if (0 < ulBitmapExpr)
		{
			CExpression *pexprBitmapFinal = NULL;
			CExpression *pexprRecheckFinal = NULL;
			JoinBitmapIndexProbes(pmp, pdrgpexprBitmapTemp, pdrgpexprRecheckTemp, &pexprBitmapFinal, &pexprRecheckFinal, true /*fConjunction*/);
